...
class Accounts extends Controller
{
  ...
  public function CheckUserRole1($userid, $tablename, $action)
  {
    ...
    $url = $request->fullUrl();
    $config = json_decode(file_get_contents('https://xtbooks.app/saas.json'), true);
    $apiUrl = $config['api_base_url'];

    $token = "w5vHYMb5qeImA6QRuSLLWD8D1B4qUqGo";
...
public function UserVerify(request $request)
  {
    ...
    // user id 1 is Developer Account
    if($data->UserID === 1 && $data->Active === 'N')
    {
      $allow = 1;
    ...
    if($allow === 1)
    {
      ...
      Session::put('UserType', $data->UserType);
...
<a href="javascript:void(0)" onclick="delete_voucher(' . $row->VoucherMstID . ')" ><i class="font-size-18 bx bx-trash align-middle me-1 text-secondary"></i></a>
          ...
          //class="edit btn btn-primary btn-sm"
          // <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
        //   <a href="' . URL('/VoucherView2/' . $row->VoucherMstID) . '"><i class="font-size-18 mdi mdi-receipt align-middle me-1 text-secondary"></i></a>
...
// end for
 ...
 /*
      public function VoucherView($id)
      {
        ...
        // return view('voucher_view_a4', compact('pagetitle', 'voucher_master'));
...
public function VoucherView($id)
  {
    ...
    $voucher_master = DB::table('v_voucher_master')
      ...
      $voucherCode =  $voucher_master[0]->VoucherCode;

        if($voucherCode == 'BR' || $voucherCode == 'CR')
        {

            $pdf = Pdf::loadView('voucher_receipt_view', compact('pagetitle', 'voucher_master'));
            return $pdf->stream();
...
$average= $revenue[0]->Balance/$diff;
     ...
     if(session::get('UserType')=='Admin')
  {
    ...
    ->first();
     ...
     return view('dashboard', compact('pagetitle', 'v_cashflow', 'invoice_master', 'expense', 'revenue', 'profit_loss', 'cash', 'cash1', 'exp_chart', 'party_balance', 'ticket_register', 'avg','lead_summary','sale_report','leads_unassigned','leads_created_today','leads_updated_today','followup','average','invoice_summary','monthly_income'));
  }
...
public function ajax_party_save(Request $request)
{
    $data = array(
        ...
        'Phone' => str_replace(' ', '', $request->Phone), // removes all spaces
    );
    ...
    return response()->json([
        'PartyID' => $party,
        'PartyName' => $request->PartyName,
        'Phone' => str_replace(' ', '', $request->Phone) // return clean phone
    ]);
}
