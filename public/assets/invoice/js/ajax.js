...
$(".addmore").on('click',function(){
	...
	html += '<td><select name="ItemID0[]" id="ItemID0_'+i+'" class="formselect changesNoo"><option value="a|6">Select Item</option><option value="b|7">b</option></select><input type="text" name="ItemID[]" id="ItemID_'+i+'"></td>';
	...
	html += '<td><select name="SupplierID[]" id="SupplierID_'+i+'" class="form-select"><option value="">Select Supplier</option><option value="">b</option></select></td>';
...
$('#quantity_'+id[1]).val( TaxResult ) ;
	$('#total_'+id[1]).val( ( parseFloat(Fare)+parseFloat(Service)+parseFloat(OPVAT)+parseFloat(IPVAT)+parseFloat(TaxResult) ).toFixed(2) );	
	calculateTotal();
});
