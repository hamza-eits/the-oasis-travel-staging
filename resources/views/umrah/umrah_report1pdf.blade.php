...
<th width="3">Umrah <br> Fare</th>
<th width="10">Fare</th>
<th width="10">Serice</th>
<th width="10">VAT5%</th>
...
@endphp
 ...
 <td >{{$invoice_detail->InvoiceMasterID}}</td>
 <td >{{$invoice_detail->SalemanName}}</td>
 <td >{{$invoice_detail->PaxName}}</td>
  <td >{{$invoice_detail->Contact}}</td>
 ...
 <td  align="center">{{number_format($invoice_detail->UmrahFare,2)}}</td>
 <td  align="center">{{number_format($invoice_detail->Fare,2)}}</td>
 <td  align="center">{{number_format($invoice_detail->Service,2)}}</td>
 <td  align="center">{{number_format($invoice_detail->Taxable,2)}}</td>
 <td  align="center">{{number_format($invoice_detail->Total,2)}}</td>
 <td  align="center">{{number_format($invoice_detail->Paid,2)}}</td>
 <td  align="center">{{number_format($invoice_detail->PaymentInBus,2)}}</td>
 </tr>
 @endforeach   
 <tr style="background-color: #e9e9e9; font-weight: bolder;">
  <td colspan="10" align="center"><strong>Total ({{$no}} Umrah)</strong></td>
  <td align="center">{{number_format($totalUmrahFare,2)}}</td>
  <td align="center">{{number_format($totalFare,2)}}</td>
  <td align="center">{{number_format($totalService,2)}}</td>
  <td align="center">{{number_format($totalVAT,2)}}</td>
  <td align="center">{{number_format($totalInvoice,2)}}</td>
  <td align="center">{{number_format($totalPaid,2)}}</td>
  <td align="center">{{number_format($totalPaymentInBus,2)}}</td>
</tr>
...
<br>
Payment Received : {{number_format($totalPaid)}} <br>
Collection in Bus : {{number_format($totalPaymentInBus)}}
