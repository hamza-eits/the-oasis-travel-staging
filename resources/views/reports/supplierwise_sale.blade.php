    ...
    <div class="container-fluid">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <form action="{{URL('/SupplierWiseSale1')}}" method="post" name="form1" id="form1"> {{csrf_field()}}
...
<script>
  $(document).ready(function(){
    $('#pdf').click(function(event){
        ...
        $('#form1').attr('action', '{{URL("/SupplierWiseSale1PDF")}}');
        $('#form1').submit();
    });
    $('#online').click(function(event){
        event.preventDefault();
        $('#form1').removeAttr('target');
        $('#form1').attr('action', '{{URL("/SupplierWiseSale1")}}');
        $('#form1').submit();
    });
});
</script>
